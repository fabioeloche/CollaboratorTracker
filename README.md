...
| **General Input Validation**  | Validates input fields across all functions to ensure no fields are left blank or contain only spaces | Intentionally left input fields blank or filled them with spaces in various functions to test error handling. | Software consistently identified empty or space-only inputs and prompted for valid data entry. | Pass      |
| **Log Task - Incorrect Data Format** | Logs a task entry with all required fields                       | Entered data in an incorrect format; software flagged the error and prompted for correction. Re-entered data in the correct format successfully. | Software identified incorrect format and guided the user to correct it. | Pass      |
| **Log Task - Task Type Selection Error** | Allows user to select a task type from a predefined menu         | Selected an invalid option (e.g., 4 instead of 1, 2, 3). Software flagged the error and prompted for a valid selection. | Software correctly identified invalid selection and prompted user to select a valid option. | Pass      |
| **View Statistic - Select Month Error** | Allows user to select a valid month from the list of 12 options   | Selected an invalid value (e.g., 40) to test the system's response. | Software flagged the error and prompted the user to select a valid month from the list. | Pass      |
